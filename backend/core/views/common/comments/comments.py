...
def _comments_file(view_kind: str) -> Path:
    view_kind = view_kind.lower()
    return COMMENTS_DIR / f"{view_kind}_comments.json"


def load_comments(view_kind: str) -> dict:
...
def save_comments(view_kind: str, data: dict) -> None:
    path = _comments_file(view_kind)
...
def get_comments_for(view_kind: str, numero: str) -> list[dict]:
    all_comments = load_comments(view_kind)
    return list(all_comments.get(str(numero), []))


def append_comment(view_kind: str, numero: str, text: str, author: str | None = None) -> list[dict]:
    numero = str(numero)
    all_comments = load_comments(view_kind)
