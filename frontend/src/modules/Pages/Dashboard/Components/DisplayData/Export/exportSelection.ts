...
export function exportSelectionToExcel(selected: Item[], visibleColumns: string[]) {
  if (!selected || selected.length === 0) return;

  const filtered = selected.map((item: Item) => {
  ...
  utils.book_append_sheet(wb, ws, 'Selected Items');
  writeFileXLSX(wb, 'selected_items_export.xlsx');
}